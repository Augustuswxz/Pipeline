...
# 统计了缺陷统计信息，会根据缺陷统计信息进行分类确定置信度阈值
class WeldAlignment:
    """环焊缝对齐结果的数据结构"""
    ...
    def get_nearest_aligned_weld2(self, target_distance: float) -> Optional[Dict]:
        ...
        for alignment in self.alignments:
            # print(alignment['file2_distance'])
...
def read_weld_data(df):
    ...
    for col in df.columns:
        ...
        if 'log distance' in col_str:
            abs_distance_col = col
        elif 'upstream girth weld' in col_str:
            weld_number_col = col
        elif 'feature type' in col_str:
            weld_type_col = col
        elif 'peak depth' in col_str:
            depth_col = col
...
def read_defect_data(df, file_type=1):
    ...
    for col in df.columns:
        col_str = str(col).lower()
        if 'upstream girth weld' in col_str:
            weld_number_col = col
        elif 'log distance' in col_str:
            abs_distance_col = col
        elif 'clock position' in col_str:
            orientation_col = col
        elif 'peak depth' in col_str:
            depth_col = col
        elif 'length' in col_str and 'mm' in col_str:
            length_col = col
        elif 'width' in col_str and 'mm' in col_str:
            width_col = col
        elif 'feature type' in col_str:
            comment_col = col
        elif 'feature identification' in col_str:
            defect_type_col = col
...
def classify_defect_type(comment):
    ...
    if 'mfg' in comment_lower or '制造' in comment_lower:
        ...
        return '机械损伤'
    elif '凹陷' in comment_lower:
        return '凹陷'
    else:
        return str(comment_lower)


def analyze_defect_distribution(defects1: List[Dict], defects2: List[Dict]) -> float:
    """
    分析缺陷分布
    """
    print("正在分析缺陷分布模式...")
    ...
    # 合并两个文件的缺陷数据
    all_defects = defects1 + defects2

    if not all_defects:
        print("未找到缺陷数据，使用默认置信度阈值: 0.6")
        return 0.6
    ...
    for defect in all_defects:
        distance = defect['distance_to_weld']
        orientation = defect['clock_position']

        # 过滤无效数据
        if distance >= 0 and 0 <= orientation <= 360:
            distances.append(distance)
            orientations.append(orientation)

    if len(distances) < 10:
        print(f"缺陷数据过少 ({len(distances)}个)，使用默认置信度阈值: 0.6")
        return 0.6
    ...
    # 计算分布密度统计量
    density_metrics = calculate_distribution_density(distances_array, orientations_array)

    return density_metrics

@tool
def calculate_confidence_threshold(density_metrics):
    """
    设置置信度阈值
    """
    return set_confidence_by_density(density_metrics)


def calculate_distribution_density(distances: np.ndarray, orientations: np.ndarray) -> Dict[str, float]:
    """
    计算缺陷分布密度统计量
    """
    metrics = {}

    # 1. 密度峰值比 - 反映数据是否有明显的聚集区域
    try:
        # 使用二维直方图计算密度
        hist2d, x_edges, y_edges = np.histogram2d(distances, orientations, bins=20)
        max_density = np.max(hist2d)
        mean_density = np.mean(hist2d)
        metrics['density_ratio'] = max_density / mean_density if mean_density > 0 else 1.0
    except:
        metrics['density_ratio'] = 1.0

    # # 2. 最近邻距离 - 反映缺陷之间的空间距离
    # try:
    #     # 简化版最近邻距离计算
    #     from scipy.spatial import distance
    #     coords = np.column_stack([distances, orientations])
    #     # 随机采样部分点计算最近邻距离，避免计算量过大
    #     if len(coords) > 100:
    ...
    #
    #     # 计算所有点对之间的距离
    #     dist_matrix = distance.cdist(coords_sample, coords_sample)
    #     # 将对角线设为无穷大（自身距离）
    ...
    #     print(f"最近邻计算失败: {e}")
    #     metrics['mean_nn_distance'] = 0.1
    #     metrics['min_nn_distance'] = 0.05

    # 3. 距离方差 - 反映缺陷在距离维度上的分散程度
    metrics['distance_variance'] = np.var(distances)

    # 4. 方位方差 - 反映缺陷在时钟方位上的分散程度
    metrics['orientation_variance'] = np.var(orientations)

    # 5. 聚类指数 - 反映数据分布的集中程度 (0-1，越大表示越集中)
    try:
        # 使用分箱计算分布均匀性
        hist2d, _, _ = np.histogram2d(distances, orientations, bins=10)
        prob = hist2d / np.sum(hist2d)
        prob_flat = prob.flatten()
        prob_flat = prob_flat[prob_flat > 0]  # 移除0概率

        entropy = -np.sum(prob_flat * np.log2(prob_flat))
        max_entropy = np.log2(100)  # 100个箱子
        metrics['clustering_index'] = 1 - (entropy / max_entropy)

    except:
        metrics['clustering_index'] = 0.5

    # 6. 综合密度评分 - 综合考虑所有指标的加权评分
    metrics['composite_density_score'] = calculate_composite_density_score(metrics)

    # 输出统计量并添加详细解释
    print("\n=== 缺陷分布密度统计量解释 ===")
    print(f"密度峰值比: {metrics['density_ratio']:.3f} - 反映数据聚集程度，值越大表示缺陷越集中在少数区域")
    # print(f"平均最近邻距离: {metrics['mean_nn_distance']:.3f} - 缺陷间的平均距离，值越小表示分布越密集")
    # print(f"最小最近邻距离: {metrics['min_nn_distance']:.3f} - 最接近的缺陷对距离，反映局部密集程度")
    print(f"距离方差: {metrics['distance_variance']:.3f} - 缺陷在距离维度上的分散程度，值越大表示距离分布越广")
    print(f"方位方差: {metrics['orientation_variance']:.3f} - 缺陷在时钟方位上的分散程度，值越大表示方位分布越广")
    print(f"聚类指数: {metrics['clustering_index']:.3f} - 数据集中程度(0-1)，值越大表示缺陷越集中")
    print(f"综合密度评分: {metrics['composite_density_score']:.3f} - 整体分布密集程度的综合评分(0-1)")

    return metrics


def calculate_composite_density_score(metrics: Dict[str, float]) -> float:
    """
    计算综合密度评分 (0-1)，值越大表示分布越密集
    """
    # 各指标的权重
    weights = {
        'density_ratio': 0.6,  # 密度峰值比 - 越高越密集
        # 'mean_nn_distance': 0.25,  # 平均最近邻距离 - 越低越密集
        # 'min_nn_distance': 0.2,  # 最小最近邻距离 - 越低越密集
        'clustering_index': 0.4  # 聚类指数 - 越高越密集
    }

    # 归一化各项指标
    normalized_scores = {}

    # density_ratio: 通常范围1-10，大于3表示有明显聚集
    normalized_scores['density_ratio'] = min(metrics['density_ratio'] / 5.0, 1.0)

    # # mean_nn_distance: 归一化距离
    # max_distance = max(metrics['mean_nn_distance'], 0.3)  # 避免除0
    # normalized_scores['mean_nn_distance'] = max(0, 1 - (metrics['mean_nn_distance'] / max_distance))
    #
    # # min_nn_distance: 归一化距离
    # max_min_distance = max(metrics['min_nn_distance'], 0.15)
    # normalized_scores['min_nn_distance'] = max(0, 1 - (metrics['min_nn_distance'] / max_min_distance))

    # clustering_index: 已经是0-1范围
    normalized_scores['clustering_index'] = metrics['clustering_index']

    # 计算加权综合评分
    composite_score = 0
    for key, weight in weights.items():
        composite_score += normalized_scores[key] * weight

    return min(1.0, max(0.0, composite_score))


def set_confidence_by_density(density_metrics: Dict[str, float]) -> float:
    """
    根据密度统计量设置置信度阈值
    """
    composite_score = density_metrics['composite_density_score']
    ...
    print(f"\n=== 置信度阈值设置 ===")
    print(f"分布密度等级: {density_level}")
    print(f"综合密度评分: {composite_score:.3f}")
    print(f"推荐置信度阈值: {confidence:.3f}")
    print(f"说明: {density_level}分布使用{'严格' if confidence > 0.6 else '宽松'}的匹配标准")
...
def comprehensive_weld_alignment(weld_numbers1: List[str], absolute_distances1: List[float], redis1: List[float],
    ...
    while i < len1 and j < len2:
        ...
        if is_similar(accum1, accum2, similarity_threshold) and len(segment1) > 0 and len(segment2) > 0:
            ...
            # 如果段落匹配失败，尝试单个环焊缝匹配
            if align_single_weld(result, weld_numbers1[i], absolute_distances1[i], redis1[i],
                                 weld_numbers2[j], absolute_distances2[j], redis2[j], abs_threshold):
                # 单个环焊缝匹配成功
                i += 1
                j += 1
            else:
...
def align_segments(result, weld_numbers1, absolute_distances1, redis1,
    ...
    while i < len(segment1) and j < len(segment2):
        if abs(current_redis1 - current_redis2)/current_redis1 < rel_threshold \
            ...
            # print(absolute_distances2[segment2[j]])
            i += 1
            j += 1
...
def align_defects_with_comprehensive_mapping(defects1: List[Dict], defects2: List[Dict],
                                             weld_alignment: WeldAlignment, thresholds: Dict = None, min_confidence = 0.6) -> pd.DataFrame:
    """
    ...
    for defect1 in defects1:
        ...
        if nearest_aligned:
            ...
            if best_match and best_confidence >= min_confidence * 0.8:  # 降低阈值
                matched_defects2.add(id(best_match))
                match_type = "相对距离匹配"
                result_df = _append_defect_alignment_result(
                    result_df, defect1, best_match, best_confidence, best_explanation, match_type
    ...
    for defect2 in defects2:
        if id(defect2) not in matched_defects2:
            ...
            if mapped_weld1:
                ...
                # 尝试基于相对距离匹配
                defect2_abs_distance = defect2.get('absolute_distance', 0)
                # print(type(defect2_abs_distance))
...
def _append_defect_alignment_result(df, defect1, defect2, confidence, explanation, match_type):
    ...
    """
    new_row = {
        '文件1缺陷ID': defect1['original_index'] if defect1 else '',
        ...
        '文件2缺陷ID': defect2['original_index'] if defect2 else '',
...
def find_best_weld_alignment(weld_nums1_str, abs_dist1, rel_dist1, weld_nums2_str, abs_dist2, rel_dist2):
    ...
    for base_distance in base_distances:
        ...
        # print(aligned_count, max_aligned_count)
        # print(aligned_count > max_aligned_count)
...
@tool
def step1_analyze_pipeline_data(filename1: str, filename2: str):
    """
    第一阶段：数据读取与初步分析
    ...
    2. 进行环焊缝对齐。
    3. 读取缺陷数据。
    4. 分析缺陷分布并生成 metric 向量。
    
    Returns:
        dict: 包含所有中间数据的字典，如果出错则返回 None。
              包含 keys: 'metric', 'defects1', 'defects2', 'weld_alignment', 
                        'filename1', 'filename2', 'best_base_distance', 'df1', 'df2'
        str: 错误信息（如果成功则为 None）
    """
    ...
    # 检查文件
    if not os.path.exists(file1_path): return None, f"错误：找不到文件 {filename1}"
    if not os.path.exists(file2_path): return None, f"错误：找不到文件 {filename2}"
    ...
    try:
        ...
        # 统计对齐结果（用于打印日志，实际数据存在 weld_alignment 中）
        aligned_count = weld_alignment.aligned_count
        ...
        # 分析缺陷分布并获取分布向量 metric
        # --- 这是分割点 ---
        metric = analyze_defect_distribution(
            defects1, defects2
        )
        ...
        context_data = {
            ...
            'defects1': defects1,
            'defects2': defects2,
            'weld_alignment': weld_alignment,
            'best_base_distance': best_base_distance,
            'filename1': filename1,
            'filename2': filename2,
            'aligned_count': aligned_count # 传递统计数据方便后续使用
        }
        ...
        print(f"第一阶段处理错误: {e}")
        traceback.print_exc()
        return None, f"第一阶段处理错误: {str(e)}"

@tool
def step2_generate_alignment_report(context_data: dict, thresholds: dict, min_confidence: float, save_type: str, output_dir=None):
    """
    第二阶段：报告生成
    
    Args:
        context_data (dict): 第一阶段返回的数据字典。
        min_confidence (float): 外部计算好的置信度阈值。
        save_type (str): 保存文件的类型，默认或专家。
    ...
    # 解包数据
    # metric = context_data['metric'] # metric在此处不再直接用于计算阈值，仅用于传递或记录
    defects1 = context_data['defects1']
    defects2 = context_data['defects2']
    weld_alignment = context_data['weld_alignment']
    filename1 = context_data['filename1']
    filename2 = context_data['filename2']
    best_base_distance = context_data['best_base_distance']
    aligned_count = context_data.get('aligned_count', 0)
    ...
    output_filename = f"aligned_{os.path.splitext(os.path.basename(filename1))[0]}_{os.path.splitext(os.path.basename(filename2))[0]}_{save_type}.xlsx"
    output_path = os.path.join(output_dir, output_filename)

    try:
        print(f"\n使用传入的五维阈值: {thresholds}")
        print(f"\n使用传入的置信度阈值: {min_confidence:.3f}")

        # 使用优化后的阈值进行缺陷对齐
        print("正在进行缺陷对齐...")
        defect_alignment_df = align_defects_with_comprehensive_mapping(
            defects1, defects2, weld_alignment, thresholds, min_confidence
        )
        ...
        return f"处理完成。文件已生成: [FILE:{output_path}]"

    except Exception as e:
        print(f"第二阶段处理错误: {e}")
        traceback.print_exc()
        return f"第二阶段处理错误: {str(e)}"